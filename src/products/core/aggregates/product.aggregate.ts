...
export class ProductAggregate {
  ...
  private assertSingleForeignKey(price: ProductPriceObjectValue): void {
    const fkCount = [price.id_client, price.id_location, price.id_route_day].filter(Boolean).length;
    if (fkCount > 1) {
      throw new Error(
        'A price may only have one foreign key (id_client, id_location, or id_route_day).',
      );
    }
  }
  ...
  private assertNoDuplicateEntity(price: ProductPriceObjectValue): void {
    const isBase = price.id_client && price.id_location && price.id_route_day;

    const duplicate = this._prices.find((p) => {
      if (isBase) {
        return p.id_client && p.id_location && p.id_route_day;
      }
      if (price.id_client && p.id_client === price.id_client) return true;
  ...
  updateProduct(
    ...
    if (this._product === null) throw new Error('Product is not initialized.');
    if (this._product.product_status === PRODUCT_STATUS_ENUM.INACTIVE) 
      throw new Error(`Cannot perform this operation. Product "${this._product.id_product}" is inactive.`);
