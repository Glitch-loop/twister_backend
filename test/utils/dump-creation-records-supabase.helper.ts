...
export async function createDumpRecordInDatabase(client: SupabaseClient, id: string, record_type: dumpRecordType, payload: object): Promise<dumpRecordInterface> {
  ...
  try {
    ...
    if (record_type === 'products') await client.from('products').insert(payload);
    if (record_type === 'users') await client.from('users').insert(payload);
    
    return { id, record_type, payload };
  } catch (error) {
    throw new Error(`An error has happended during dump record creation: ${error instanceof Error ? error.message : String(error)}`);
  }
}

export function createDumpRecord(id: string, record_type: dumpRecordType, payload: object): dumpRecordInterface {
  return { id, record_type, payload };
}
